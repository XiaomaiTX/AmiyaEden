...
export function InfoFittingsPage() {
  ...
  const groupOptions = useMemo(() => {
    return Array.from(
      new Set((data?.fittings ?? []).map((item) => item.group_name || 'Unknown'))
  ...
  const filtered = useMemo(() => {
    ...
    return (data?.fittings ?? [])
      ...
      .filter((item) => {
        ...
        return [item.name, item.ship_name, item.group_name].some((value) =>
    ...
    <section className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-lg border bg-card p-4">
        ...
        >
          ...
          <SelectContent>
            <SelectItem id="">{t('infoFittings.allRaces')}</SelectItem>
            {raceOptions.map((item) => (
              <SelectItem key={item.id} id={String(item.id ?? '')}>
                {item.name}
              </SelectItem>
            ))}
          </SelectContent>
        ...
        >
          ...
          <SelectContent>
            <SelectItem id="">{t('infoFittings.allGroups')}</SelectItem>
      ...
      {!loading && grouped.length === 0 ? (
        <p className="text-sm">{t('infoFittings.empty')}</p>
      ) : null}
          ...
          return (
            <Collapsible
              ...
              onExpandedChange={(open) =>
                setCollapsedGroups((previous) => ({ ...previous, [groupName]: !open }))
              }
