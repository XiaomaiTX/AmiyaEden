...
func buildCaptainRewardSettlementBaseQuery(db *gorm.DB, captainUserID *uint) *gorm.DB {
	query := db.Model(&model.CaptainRewardSettlement{})
	if captainUserID != nil && *captainUserID > 0 {
		query = query.Where("captain_user_id = ?", *captainUserID)
	}
	return query
}
...
func buildCaptainRewardSettlementListQuery(
	...
	return buildCaptainRewardSettlementBaseQuery(db, captainUserID).
...
func (r *CaptainRewardSettlementRepository) List(
	...
	var total int64
	db := buildCaptainRewardSettlementBaseQuery(global.DB, captainUserID)
	if err := db.Count(&total).Error; err != nil {
		return nil, 0, err
	}
	err := buildCaptainRewardSettlementListQuery(global.DB, captainUserID, page, pageSize).
		Find(&rows).Error
...
func (r *CaptainRewardSettlementRepository) Summarize(
	...
	var result row
	err = buildCaptainRewardSettlementBaseQuery(global.DB, captainUserID).
		Select(`
