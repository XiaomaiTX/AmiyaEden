...
type CorpStructureInfo struct {
	...
	// ServiceModules is the authoritative service-slot snapshot from the corporation
	// assets endpoint. ESI's structures endpoint exposes activity labels only.
	ServiceModules      string  `gorm:"type:text"  json:"service_modules"`
	ServiceModulesKnown bool    `gorm:"not null;default:false" json:"service_modules_known"`
