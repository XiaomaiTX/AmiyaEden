...
func RegisterAll(reg *taskregistry.Registry) {
	...
	registerGalaxyRegistryValidationTask(reg)
	registerStructureFuelRateSyncTask(reg)
	registerCorporationStructureAlertScanTask(reg)
	registerQQGovernanceTasks(reg)
