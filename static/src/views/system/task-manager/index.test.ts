...
const esiControlsSource = readFileSync(new URL('./modules/EsiControls.vue', import.meta.url), 'utf8')
...
const routerSource = readFileSync(new URL('../../../router/modules/system.ts', import.meta.url), 'utf8')
...
test('task manager page uses tabs with extracted task and history modules', () => {
	...
	assert.match(esiControlsSource, /fetchESIRefreshStatuses/)
	assert.match(esiControlsSource, /runESIRefreshTask\(/)
	...
	assert.match(esiControlsSource, /taskManager\.esi\.sections\.statuses/)
...
test('task manager types and locales exist and the legacy page is removed', () => {
	...
	assert.match(typeSource, /type TaskHistoryList = Api\.Common\.PaginatedResponse<TaskExecutionItem>/)
