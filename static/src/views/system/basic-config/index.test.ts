...
test('basic config page consumes enforced and legacy capabilities from backend', () => {
  assert.match(typeSource, /enforced_capabilities:\s*string\[\]/)
  assert.match(typeSource, /legacy_capabilities:\s*Record<string, string\[\]>/)
  assert.match(source, /enforcedCapabilitySet/)
  assert.match(source, /legacyCapabilities/)
  assert.match(source, /visibleCorpCapabilityGroups/)
  assert.match(source, /res\.enforced_capabilities/)
  assert.match(source, /res\.legacy_capabilities/)
  assert.match(source, /selectedLegacyCapabilities/)
  assert.match(source, /corp-policy-legacy-alert/)
  assert.match(zhLocaleSource, /"corpPolicyLegacyTitle"\s*:/)
  assert.match(zhLocaleSource, /"corpPolicyLegacyDescription"\s*:/)
  assert.match(enLocaleSource, /"corpPolicyLegacyTitle"\s*:/)
  assert.match(enLocaleSource, /"corpPolicyLegacyDescription"\s*:/)
})
