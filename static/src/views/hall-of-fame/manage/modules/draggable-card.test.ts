...
test('draggable card exists and uses interactjs around the shared hero card', () => {
  ...
  assert.match(source, /zoomRatio: number/)
  assert.match(source, /event\.dx \/ effectiveZoomRatio\.value/)
  assert.match(source, /event\.rect\.width \/ effectiveZoomRatio\.value/)
})
