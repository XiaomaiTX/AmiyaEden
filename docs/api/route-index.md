...
| POST   | `/dashboard/galaxy-registry/entries/:id/end`                | 结束当前队长自己的 active 登记；服务层会逐个尝试拉取当前绑定角色的钱包 ESI 并立即写回校验结果；部分角色 token 失效时跳过并继续，全部不可用时返回失败并保持 pending | `RequireRole(captain)`      |
...
| POST   | `/dashboard/galaxy-registry/admin/entries/:id/revalidate`   | 对已结束登记重新触发校验；逐个尝试拉取当前绑定角色钱包 ESI 并覆盖现有结果；部分角色 token 失效时跳过并继续，全部不可用时返回失败并保留 pending | `RequireRole(admin)`        |
