...
- `/system/shop/product/*` 默认要求 `admin`
- `/system/shop/order/*` 默认要求 `admin` 或 `shop_order_manage`
...
- **我的订单**（`shop/browse` → 我的订单 Tab）：展示订单状态，以及在已发放/已拒绝时展示操作人；同时展示下单备注，并在任一订单带有发放/拒绝备注时额外展示“处理备注”列。
