...
- 主殿页面向所有 `Login` 用户展示名人堂画布、背景图与全部可见卡片
- 管理页面向 `admin` 开放，支持新增、编辑、删除、拖拽摆放、修改卡片尺寸，以及编辑卡片文案、人物 ID、主题、称号颜色、字体大小和头像下方小图
...
- 主殿页面不再显示顶部标题区，画布按真实尺寸渲染并支持水平滚动浏览；卡片支持悬浮发光效果
- 管理页画布支持缩放编辑，便于在超大画布上精细摆放卡片
- 管理页预览会把当前未保存的本地草稿写入浏览器存储，再在新标签页中按草稿内容渲染主殿预览
...
- `static/src/views/hall-of-fame/temple` — 主殿页（所有已登录用户）
