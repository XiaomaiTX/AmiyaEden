...
- 队长点击结束生产时，服务层会逐个尝试拉取该队长当前绑定角色的钱包 ESI，并立刻写回最终校验结果
- 部分角色 token 失效时，跳过失效角色，继续用其余角色完成校验
- 若全部绑定角色都无法完成钱包刷新，则登记已结束但校验保持未完成（`pending`）
- 非 token 类错误（如 ESI 临时故障、数据库异常）仍然中断整次校验，避免误吞系统故障
...
- `admin` 还可对已结束登记重新触发校验；该操作会逐个尝试拉取当前绑定角色钱包 ESI，并用最新重算结果覆盖现有校验结果；部分角色 token 失效时跳过失效角色
