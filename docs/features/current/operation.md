...
- 发放 PAP 时的伏羲币奖励受多人物奖励限制：同一用户在同一舰队中的多个人物按配置递减奖励，详见 `docs/features/current/pap-exchange.md`
- 舰队 `pap_count` 统一要求大于 `0` 且按 `0.5` 粒度；创建、编辑、发放都会执行该校验。历史不符合粒度的舰队不会被自动修正，需先编辑到合法值后才能发放。
