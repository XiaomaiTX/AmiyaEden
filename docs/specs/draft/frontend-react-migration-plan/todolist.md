...
1. 完成全量跨角色回归：覆盖普通成员、管理员、受限角色和关键写操作。
2. 完成 React/Vue 契约、权限语义与中英文文案的持续对齐。
3. 优先将高风险复杂表格、表单和对话框迁入共享 shadcn/ui 基元；其余页面按债务清单分批收敛。
4. 维护双镜像部署说明、路由范围基线和组件债务状态。
...
- [x] Vue `frontend` 与 React `frontend-react` 独立镜像并行部署
- [ ] 建立并维护跨角色回归记录与双端契约对齐清单
- [ ] 收敛共享 shadcn/ui 表格、表单、对话框与选择器模式
- [ ] 按 `ui-standardization-backlog.md` 分批清理原生组件债务
